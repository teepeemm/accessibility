...
%\begin{document}
%\begin{frame}
%onslide:\onslide<2->{\includegraphics[width=5em]{example-image.pdf}\includegraphics[width=5em,page=2]{accessibleDuck}}\qquad
%   only:\only   <2->{\includegraphics[width=5em]{example-image.pdf}\includegraphics[width=5em,page=2]{accessibleDuck}}\\\bigskip
%uncover:\uncover<2->{\includegraphics[width=5em]{example-image.pdf}\includegraphics[width=5em,page=2]{accessibleDuck}}\qquad
%visible:\visible<2->{\includegraphics[width=5em]{example-image.pdf}\includegraphics[width=5em,page=2]{accessibleDuck}}
%\end{frame}
%\end{document}
...
\texttt{math/alt/use} $\Leftrightarrow$ Blackboard\\
Optional warmups: \verb|\RequirePackage{nag}| xor\\
...
\item\texttt{\textbackslash tagpdfsetup\{table/multirow=\{...\}\}}
...
\item Some packages need some (or a lot of) help (tcolorbox, titlesec, qrcode, media9)
\end{itemize}
\end{frame}
...
\item ``\emph{highly} experimental'' --- must use \texttt{lualatex-dev}
\item Can ``easily'' specify header and footer information\\\bigskip
\hspace{.3\textwidth}%
%\begin{uncoverenv}<+->
\onslide<+->{\parbox[c][.4\textheight]{0pt}{ }}%
\only<+->{\includegraphics[height=.4\textheight,alt={MALL presentation with logo in corner}]{Blackboard-PPT-Tues.pdf}}%
%\end{uncoverenv}
\end{itemize}
\end{frame*}
...
\item Much longer compilation (Talon), needs more memory
\item \url{https://www.youtube.com/watch?v=Eu_qM53tInw&t=5900s} A before and after example
\end{itemize}
\end{frame}
...
Math Track Meet Dec 2023 & Y & Y & Y & Y & Y & Y \\
Math Track Meet 2025 & Y & Y & Y & Y & Y & Y \\
Calculus & Y & Y & N & Y\mynote{***} & N & \\
Linear Algebra & Y & ``Y'' & Y\mynote* & Y\mynote* & Y\mynote* & Y\mynote* \\
this talk & Y & ``Y'' & Y\mynote* & Y\mynote* & Y\mynote* & N \\
MALL syllabus${}\times37$ & Y & ``Y'' & Y\mynote* & Y\mynote* & Y\mynote* & Y\mynote* \\\addlinespace
Simple Syllabus & N & ``N'' & N & N & N & N/A \\\bottomrule
...
\phantom{Calculus: }UA-2: caption, intertext\{math\}, media9
\end{minipage}
\end{frame}

\begin{frame}
\frametitle{Promised Resources}\centering
\url{https://sites.und.edu/timothy.prescott/accessible}\\\bigskip
\qrcode{https://sites.und.edu/timothy.prescott/accessible}
\end{frame}

...
(If a package in this category
%causes
errors because you use \cs{DocumentMetadata} solely
to adjust the \PDF\ metadata,
you can use the \texttt{pdfmanagement} package
instead.)
% of \cs{DocumentMetadata}.)
...
repository at \url{https://github.com/latex3/tagging-project}.
...
\begin{tikzcd}[
    ...
    column sep=tiny,
    cramped,
    alt={A commutative diagram where ker(a) → ker(b) → ker(c) →(d) coker(a) → coker(b) → coker(c), A →(f) B →(g) C → 0, 0 → A' →(f') B' →(g') C', ker(a) → A →(a) A' → coker(a), ker(b) → B →(b) B' → coker(b), and ker(c) → C →(c) C' → coker(c).} % 51 words, 274 char
  ]
  \& \& \ker(a) \ar{r} \ar{d}
  \& \ker(b)\ar{r} \ar{d}
...
\begin{figure}
\includegraphics[width=\linewidth,alt={Described by the caption.}]
{figdivthm_space2_3DBW}
...
(The alt text used by this article for the snake lemma uses 274 characters.)
...
\begin{figure}
  \begin{center}
  \begin{tikzpicture}[alt={A graph that is a line from (-2,0.5) to a hollow dot at (-1,1), a solid dot at (-1,0), the graph goes from (-1,0) upward, peaking near (-0.5,0.6), decreasing to cross the y-axis at 0.5, bottoming out at (1,0), and increasing to finish at (2,1.5).}]
...
\end{verbatim}
at the beginning of the multirow cell.
...
\item[\texttt{beamer}] Becomes \texttt{ltx-talk}.
For more details, see my other article in this issue.
(Not for the textbook, but as long as I'm mentioning package changes.)
\end{description}
...
Linux (see \url{https://tex.stackexchange.com/a/755945}).
...
2.0 listed at \url{https://pdfa.org/accessible-math-in-pdf-finally}.
% \cite{pdfCheckersURL}.
...
at \url{https://github.com/veraPDF/veraPDF-validation-profiles}.
...
These do not need to be downloaded.
% from \cite{veraProfilesURL}.
...
\autoref{vera-report} (see \url{https://tex.stackexchange.com/a/763124}),
% \cite{combineProfilesURL},
although their overlap is unclear and the \acro{WCAG} profiles may move to
\acro{PDF4WCAG} at \url{https://pdf4wcag.com/}
(see \url{https://github.com/latex3/tagging-project/discussions/1357#discussioncomment-17086425}).
%\cite{migrationURL}.
...
Additionally, \PDFix\ offers the profiles from
%\cite{veraProfilesURL}
...
headings (the Ally checklist is at \url{https://help.anthology.com/ally-lms/en/administrators/ally-accessibility-checklist.html}).
% \cite{allyChecksURL}.
...
heading, see \url{https://tex.stackexchange.com/a/758805}.
...
or \acro{ngPDF} at \url{https://ngpdf.com/}.
...
textbook (see \url{https://github.com/teepeemm/LinearAlgebra})
and a differential equations
textbook (see \url{https://github.com/teepeemm/trenchDEs})
to both be accessible.

The most work involved is when someone used manual formatting
instead of \LaTeX\ defaults (\cs{over} wasn't all that fun, either).
Typing \texttt{1. ... 2. ... 3} instead of using
...
Accessible \PDF s require more computing power to compile.
\begin{table}
...
command (see \url{https://tex.stackexchange.com/a/741777/} for details).
% \cite{moreMemoryURL}.

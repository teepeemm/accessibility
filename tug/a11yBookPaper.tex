...
\footnote{If a required package in this category
...
We can combine the profiles in \autoref{vera-report} \cite{combineProfilesURL},
although their overlap is unclear and the \acro{WCAG} profiles may move to
\acro{PDF4WCAG}\footnote{\url{https://pdf4wcag.com/}} \cite{migrationURL}.
...
Ally also requires that documents over two pages
use headings \cite{allyChecksURL}.
I normally accomplish this with \cs{section} commands,
but if a document does not otherwise have them,
it is possible to have the title also
serve as a heading \cite{titleHeadingURL}.

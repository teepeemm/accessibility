...
I often turned to the \TeX\ Stack Exchange for help.
(In more extreme cases,
I reported a bug to the tagging project's GitHub repository.)
...
resulting in many claimed ``issues'' with a valid document \cite{spuriousURL}.
%(for example, 1.7 but not 2.0 requires that formulas have alt text).
In the positive direction,
%\url{https://pdfa.org/accessible-math-in-pdf-finally/}
there are several (but currently only five)
checkers that support \PDF\ version 2.0 \cite{pdfCheckersURL}.
...
\PDF/\acro{UA-2} & 1723 & 12926774 \\
\acro{WTPDF} 1.0 Accessibility & 1719 & 12926770 \\
\acro{WTPDF} 1.0 Reuse & 1706 & 11679799 \\\bottomrule
\end{tabular}
\end{table}
...
For example, \PDFix\ presents the above validation error as in
\autoref{pdfixValError}.
\begin{figure}
...
it is possible to have the title also provide a heading \cite{titleHeadingURL}.
...
requiring me to modify the compilation command \cite{moreMemoryURL}.
...
I have also converted a linear algebra textbook and
a differential equations textbook to both be accessible.

The most work involved is when someone used manual formatting
instead of \LaTeX\ defaults.
...
This requires a revision to replace the manual formatting

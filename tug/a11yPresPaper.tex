...
\TUB\ 47:1 \cite{Wright:TB47-1:64}.
%\cite{Wright:2026:LCA}.
...
\DocumentMetadata{}\documentclass{ltx-talk}
\end{verbatim}
%\begin{verbatim}[\verbatimOptions]
%\DocumentMetadata{}
...
\begin{verbatim}[\verbatimOptions]
\DocumentMetadata{
  lang=en-US,tagging=on,pdfstandard=ua-2,
%  check-tagging-status,
  tagging-setup={
%    math/alt/use,
...
%via \texttt{texdoc ltx-talk} on a command line, or
at \url{https://ctan.org/pkg/ltx-talk}
%\cite{ltxTalkCtanURL}.
and links to several examples at
...
so that we may want to consult \texttt{texdoc lttemplates}.
...
If a frame needs to use \cs{verb} or similar to change catcodes,
...
Within a frame,
%the commands
%\cs{section}, \cs{subsection}, and \cs{subsubsection}
\cs{((sub)sub)section}
has no effect.
(The other common sectioning commands should not appear in a presentation and
are not defined.)
Outside of the frames, the sectioning commands
...
\begin{figure}
\setkeys{Gin}{width=.49\linewidth,alt={Described by the caption.},
trim=0 100 0 0,clip % get things onto one fewer page
}
...
alt={Described by the caption.},
trim=0 100 0 0,clip % try to get things onto one fewer page
]{tugFigs}}
...
This cannot be done with the existing template, but can be accomplished
by thinking of this as a regular \LaTeX\ document and using
\begin{lstlisting}
\newsavebox\logobox
\savebox\logobox{%
...
We could use
%\verb|\EditTemplateDefaults{titlepage}{talk}{!<new key values>}|
\begin{verbatim}[\verbatimOptions]
...
In addition to the default \texttt{talk}, \texttt{empty}, and \texttt{plain}
(no header or footer),
...
The class's GitHub repository is at
%\cite{ltxGHurl}.
